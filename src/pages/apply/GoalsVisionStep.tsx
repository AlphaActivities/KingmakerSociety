...
export default function GoalsVisionStep({ data, onChange, onContinue, onBack }: GoalsVisionStepProps) {
  ...
  const advance = () => {
    setLocalError('');
    if (subStep === 0 && !data.mainGoal90Days.trim()) { setLocalError('Please describe your main goal.'); return; }
    if (subStep === 1 && !data.life12Months.trim()) { setLocalError('Please describe your vision.'); return; }
    if (subStep === 2 && !data.wantBusiness) { setLocalError('Please select an option.'); return; }
    if (subStep === 3) {
      if (!data.improvementArea) { setLocalError('Please select an area.'); return; }
      onContinue();
      return;
    }
    setSubStep(s => s + 1);
  ...
  return (
    <div className="flex flex-col w-full max-w-3xl mx-auto px-4">
      ...
      <SubStepTransition stepKey={subStep}>
        ...
        {subStep === 2 && (
          <div className="w-full space-y-6">
            ...
            <SelectCards options={BUSINESS_OPTIONS} selected={data.wantBusiness} onSelect={v => onChange({ ...data, wantBusiness: v })} />
          </div>
        )}
        ...
        {subStep === 3 && (
          <div className="w-full space-y-6">
            ...
            <SelectCards options={IMPROVEMENT_OPTIONS} selected={data.improvementArea} onSelect={v => onChange({ ...data, improvementArea: v })} />
          </div>
        )}
      </SubStepTransition>
      ...
      <div className="mt-10 flex items-center space-x-4">
        ...
        <button
          onClick={advance}
          className="group flex items-center space-x-3 px-10 py-4 bg-gradient-to-r from-[#FFC300] via-[#FFD033] to-[#D4A000] text-[#0B0B0B] font-bold text-base rounded-xl shadow-[0_4px_20px_rgba(255,195,0,0.35)] hover:shadow-[0_8px_40px_rgba(255,195,0,0.55)] hover:-translate-y-0.5 transition-all duration-300"
        >
